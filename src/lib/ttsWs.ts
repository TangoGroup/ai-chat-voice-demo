...
export class TtsWsPlayer {
  ...
  private streamEndRequested = false;
  ...
  constructor(private readonly opts: TtsWsPlayerOptions) {
    ...
    this.audioEl.onended = () => {
      try {
        ...
        this.opts.onLog?.("TTS WS: calling onPlaybackEnded callback");
        this.opts.onPlaybackEnded?.();
        this.opts.onLog?.("TTS WS: onPlaybackEnded callback completed");
        // Now safe to tear down media since playback is done
        this.teardownMedia("onended");
      } catch (error) {
    ...
    this.audioEl.ontimeupdate = () => {
      try {
        ...
        if (onLog && this.audioEl.duration > 0) {
          const progress = (this.audioEl.currentTime / this.audioEl.duration) * 100;
          // Log when near end or when stopped
          if (this.audioEl.ended || progress > 95 || (this.audioEl.paused && this.audioEl.currentTime > 0)) {
            this.opts.onLog?.(`TTS WS: audioEl timeupdate - currentTime=${this.audioEl.currentTime.toFixed(2)}, duration=${this.audioEl.duration.toFixed(2)}, ended=${this.audioEl.ended}, paused=${this.audioEl.paused}, progress=${progress.toFixed(1)}%`);
          }
        }
      } catch (error) {
        this.handleError(error, "audioEl.ontimeupdate callback");
      }
    };
        ...
        this.mediaSource!.addEventListener("sourceopen", () => {
          try {
            ...
            this.mediaSource.addEventListener("sourceended", () => {
              const { onLog: logFn } = this.opts;
              if (logFn) logFn(`TTS WS: MediaSource sourceended event fired (audioEl.ended=${this.audioEl.ended}, currentTime=${this.audioEl.currentTime.toFixed(2)}, duration=${this.audioEl.duration.toFixed(2)})`);
              // Browser quirk: When MediaSource ends, audioEl.ended might be true but onended might not fire
              // Check if audio has already ended and manually trigger if needed
              if (this.audioEl.ended && !this.playbackEndedFired) {
                if (logFn) logFn("TTS WS: audioEl already ended when MediaSource ended, but onended didn't fire - manually triggering");
                this.playbackEndedFired = true;
                this.opts.onPlaybackEnded?.();
              }
              // Otherwise, audio element will naturally fire onended when playback finishes
            });
            resolve();
          } catch (e) {
      ...
      try {
        ...
        ws.onmessage = (ev: MessageEvent<string | ArrayBuffer>) => {
          try {
            ...
            if (onLog) onLog(`TTS WS <- message: audio=${!!payload.audio}, isFinal=${!!payload.isFinal}`);
            if (payload.audio) {
            ...
            if (payload.isFinal) {
              if (onLog) onLog(`TTS WS: final message received (pendingChunks=${this.pendingChunks.length}, isBufferUpdating=${this.isBufferUpdating})`);
              this.aiFinalReceived = true;
              // Request stream end, but wait for queue to drain
              if (this.useBlobFallback) {
                if (onLog) onLog("TTS WS: calling endOfStream() immediately (blob fallback)");
                this.endOfStream();
              } else {
                this.streamEndRequested = true;
                if (onLog) onLog(`TTS WS: streamEndRequested=true, pendingChunks=${this.pendingChunks.length}, isBufferUpdating=${this.isBufferUpdating}`);
                if (this.pendingChunks.length === 0 && !this.isBufferUpdating) {
                  if (onLog) onLog("TTS WS: queue empty, calling drainQueue() to trigger endOfStream");
                  this.drainQueue(); // This will trigger the endOfStream call
                } else {
                  if (onLog) onLog(`TTS WS: waiting for queue to drain (pendingChunks=${this.pendingChunks.length}, isBufferUpdating=${this.isBufferUpdating})`);
                }
              }
              ...
              try {
                if (onLog) onLog("TTS WS: calling onFinal callback");
                this.opts.onFinal?.();
                if (onLog) onLog("TTS WS: onFinal callback completed");
              } catch (error) {
        ...
        ws.onclose = (ev) => {
          ...
          if (onLog) onLog(`TTS WS: ws.onclose calling endOfStream() (mediaSource=${!!this.mediaSource}, readyState=${this.mediaSource?.readyState})`);
          this.endOfStream();
  ...
  close() {
    const { onLog } = this.opts;
    if (onLog) onLog(`TTS WS: close() called (aiFinalReceived=${this.aiFinalReceived}, mediaSource=${!!this.mediaSource}, readyState=${this.mediaSource?.readyState})`);
    try {
      this.ws?.close();
    } catch (error) {
    ...
    this.ws = null;
    // End the stream so no more data comes, but DON'T tear down media yet
    // Let audio finish playing naturally so onended can fire
    this.endOfStream();
    // Only tear down audio analysis (stop RAF)
    this.teardownAudio("close");
    // Don't call teardownMedia here - let audio finish and onended will handle cleanup
    // If audio is already done, teardownMedia will be called from onended handler
  }
  ...
  endSession() {
    const { onLog } = this.opts;
    if (onLog) onLog(`TTS WS: endSession() called (aiFinalReceived=${this.aiFinalReceived}, mediaSource=${!!this.mediaSource}, readyState=${this.mediaSource?.readyState})`);
    try {
      this.endOfStream();
    } catch (error) {
  ...
  private drainQueue() {
    ...
    if (!next) {
      // Queue is empty - if we've requested stream end, do it now
      const { onLog } = this.opts;
      if (onLog) onLog(`TTS WS: drainQueue() - queue empty (streamEndRequested=${this.streamEndRequested}, mediaSource=${!!this.mediaSource}, readyState=${this.mediaSource?.readyState})`);
      if (this.streamEndRequested && this.mediaSource && this.mediaSource.readyState === "open") {
        try {
          if (onLog) onLog("TTS WS: calling mediaSource.endOfStream()");
          this.mediaSource.endOfStream();
          if (onLog) onLog(`TTS WS: mediaSource.endOfStream() completed (readyState=${this.mediaSource.readyState})`);
          this.streamEndRequested = false;
          this.opts.onLog?.("TTS WS: MediaSource.endOfStream() called after queue drained");
        } catch (error) {
          this.handleError(error, "mediaSource.endOfStream after drain");
        }
      }
      return;
    }
  ...
  private endOfStream() {
    const { onLog } = this.opts;
    if (onLog) onLog(`TTS WS: endOfStream() called (rafId=${!!this.rafId}, mediaSource=${!!this.mediaSource}, readyState=${this.mediaSource?.readyState})`);
    if (this.rafId) {
      try {
    ...
    if (this.mediaSource && this.mediaSource.readyState === "open") {
      try {
        this.mediaSource.endOfStream();
        // MediaSource will transition to "ended" and fire sourceended event
        // Audio element will naturally fire onended when playback finishes
      } catch (error) {
      ...
      if (this.mediaSource) {
        try {
          if (this.mediaSource.readyState === "open") {
            this.mediaSource.endOfStream();
            const { onLog: logFn } = this.opts;
            if (logFn) logFn(`TTS WS: mediaSource.endOfStream() completed (readyState=${this.mediaSource.readyState})`);
            // MediaSource will transition to ended, which triggers sourceended event
            // The sourceended handler will check if audio is done
          }
        } catch (error) {
